...
**当前版本**：schema **v2.1**（2026-04-08 落地）+ chunk 6 视觉重做（2026-04-09 完成）。
...
- **[api-overview.md](api-overview.md)** — v2.1 API endpoint 巡览（按业务域分组）
...
| **代提交 / proxy submission** | 由 A 提交但 volunteerId 是 B 的记录，状态机 PENDING_CONFIRMATION，等 B 确认 |
| **AdminCenter** | admin 登录后 `/me` 渲染的系统管理界面（不是单独的路由） |
| **台账 / SupportLedger** | admin 只读的项目支援聚合视图，路径 `/review` |
