...
const validateVolunteerPayload = (payload) => {
  if (!payload.id || !/^[A-Z]+-\d{4}$/.test(payload.id)) {
    return '志愿者ID格式错误（需为如 PG-0001）';
  }
  ...
  if (['中国大陆', '中国台湾'].includes(payload.region) && !payload.province) {
    return `${payload.region}必须填写省份`;
  }
...
const getNextVolunteerId = async () => {
  const last = await Volunteer.findOne({ id: /^PG-\d{4}$/ }).sort({ id: -1 }).select('id').lean();
  const lastNum = last?.id ? Number(String(last.id).split('-')[1]) : 0;
...
const ensureAccountForVolunteer = async (volunteer, defaultPassword, usedEmailSet) => {
  const existing = await Account.findOne({ volunteerId: volunteer.id }).select('_id').lean();
  if (existing) return { created: false, accountId: String(existing._id), email: '' };

  const email = await buildGeneratedEmail(volunteer, usedEmailSet);
  usedEmailSet.add(email);
  const passwordHash = await Account.hashPassword(defaultPassword);
  ...
  return { created: true, accountId: String(account._id), email };
};
...
class AdminController {
    ...
    try {
      ...
      if (confirm !== 'RESET') {
        return res.status(400).json({
          success: false,
          error: '请提供 confirm=RESET 以确认清空数据'
        });
      }
      ...
      let admin = await Account.findOne({ email: systemEmail }).select('+passwordHash');
      if (!admin) {
        const passwordHash = await Account.hashPassword(systemPassword);
      ...
      return res.status(200).json({
        ...
        data: {
          systemAdmin: sanitizeAccount(admin),
          deleted: {
  ...
  static async importVolunteers(req, res) {
    try {
      ...
      if (!Array.isArray(parsedRows) || parsedRows.length === 0) {
        return res.status(400).json({
          success: false,
          error: '未提供可导入数据（rows或csvText）'
        });
      }

      const usedEmails = await Account.find({}).select('email').lean();
      const usedEmailSet = new Set(usedEmails.map((item) => String(item.email).toLowerCase()));
      ...
      for (let idx = 0; idx < parsedRows.length; idx += 1) {
        ...
        const update = {
          ...
          indexedStatus: payload.status,
          region: payload.region,
          indexedRegion: payload.region || '其他',
        ...
        await Volunteer.create({ id: payload.id, ...update });
        ...
        if (parseBoolean(createAccounts, true)) {
          ...
          const accountResult = await ensureAccountForVolunteer(volunteer, defaultPassword, usedEmailSet);
  ...
  static async createVolunteerWithAccount(req, res) {
    try {
      ...
      if (parseBoolean(createAccount, true)) {
        ...
        accountData = accountResult.created ? {
  ...
  static async generateMissingAccounts(req, res) {
    try {
      ...
      const volunteers = await Volunteer.find({}).select('id chineseName email').lean();
      const usedEmails = await Account.find({}).select('email').lean();
      const usedEmailSet = new Set(usedEmails.map((item) => String(item.email).toLowerCase()));

      let created = 0;
      for (const volunteer of volunteers) {
        const result = await ensureAccountForVolunteer(volunteer, defaultPassword, usedEmailSet);
      ...
      return res.status(200).json({
        ...
        message: '已完成账号生成',
        data: {
          scannedVolunteers: volunteers.length,
  ...
  static async listAccounts(req, res) {
    try {
      const accounts = await Account.find({})
        .sort({ createdAt: -1 })
        .select('email name role volunteerId isActive lastLoginAt createdAt updatedAt')
        .lean();

      const volunteerIds = accounts.map((item) => item.volunteerId).filter(Boolean);
      const volunteers = await Volunteer.find({ id: { $in: volunteerIds } }).select('id chineseName region').lean();
      ...
      return res.status(200).json({
        ...
        data: accounts.map((item) => ({
          ...
          volunteer: item.volunteerId ? volunteerMap.get(item.volunteerId) || null : null
        }))
      });
    } catch (error) {
      return res.status(500).json({
  ...
  static async updateAccount(req, res) {
    try {
      ...
      if (String(target._id) === req.user.accountId) {
        return res.status(400).json({
          success: false,
          error: '不能修改当前系统管理员自身账号'
        });
      }
      ...
      if (role && !ALLOWED_ROLES.includes(role)) {
        return res.status(400).json({
          success: false,
          error: `role必须是: ${ALLOWED_ROLES.join(', ')}`
        });
      }

      if (target.role === 'admin' && role && role !== 'admin') {
        const activeAdmins = await Account.countDocuments({ role: 'admin', isActive: true });
        if (activeAdmins <= 1) {
          return res.status(400).json({
            success: false,
            error: '系统至少需要保留一个admin账号'
          });
        }
      }
      ...
      if (target.volunteerId && /^PG-\d{4}$/i.test(target.volunteerId) && target.volunteerId !== 'PG-0000') {
        ...
        if (volunteer && typeof volunteer === 'object') {
          ...
          if (volunteer.status !== undefined) volunteerUpdate.status = String(volunteer.status).trim();
          if (volunteer.region !== undefined) volunteerUpdate.region = String(volunteer.region).trim();
          ...
          if (volunteer.services !== undefined) {
            const services = Array.isArray(volunteer.services)
              ? volunteer.services
              : String(volunteer.services || '')
                .split(/[、,，|/]/)
                .map((item) => item.trim())
                .filter(Boolean);
            volunteerUpdate.services = services;
          }
          if (volunteerUpdate.status) volunteerUpdate.indexedStatus = volunteerUpdate.status;
          if (volunteerUpdate.region) volunteerUpdate.indexedRegion = volunteerUpdate.region;
        }
        if (Object.keys(volunteerUpdate).length > 0) {
          await Volunteer.updateOne({ id: target.volunteerId }, { $set: volunteerUpdate });
        }
      }
    ...
    try {
      ...
      if (volunteerId && /^PG-\d{4}$/i.test(volunteerId) && volunteerId !== 'PG-0000') {
        const [volunteerResult, serviceResult, applicationResult, auditResult] = await Promise.all([
          Volunteer.deleteOne({ id: volunteerId }),
          NonProjectService.deleteMany({ volunteerId }),
          ServiceApplication.deleteMany({ volunteerId }),
          AuditLog.deleteMany({
            $or: [{ 'operator.id': volunteerId }, { 'submitter.id': volunteerId }]
          })
        ]);
