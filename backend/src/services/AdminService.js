...
export const resetToSystemAdmin = async ({ confirm }) => {
  ...
  return {
    ...
    deleted: { volunteers: volunteers.count, services: services.count, applications: applications.count, audits: audits.count, accounts: accounts.count },
  };
};

export const importVolunteers = async ({ rows = [], csvText = '', createAccounts = true, defaultPassword = 'Volunteer@123' }) => {
...
export const createVolunteerWithAccount = async (body) => {
  const {
    chineseName = '', englishName = '', status = '在职', region = '其他',
...
export const generateMissingAccounts = async ({ defaultPassword = 'Volunteer@123' } = {}) => {
  ...
  return { scannedVolunteers: volunteers.length, createdAccounts: created };
};
...
export const listAccounts = async () => {
  ...
  return accounts.map(account => ({ ...serializeAccount(account), volunteer: account.volunteerId ? volunteerMap.get(account.volunteerId) || null : null }));
};

export const updateAccount = async (accountId, { role, isActive, name, email, volunteer }, currentUserId) => {
  ...
  if (email) {
    ...
    const emailExists = await prisma.account.findFirst({ where: { email: normalizedEmail, id: { not: accountId } } });
    if (emailExists) return { emailTaken: true };
...
export const deleteAccount = async (accountId, currentUserId) => {
  ...
  if (volunteerId && /^PG-\d{4}$/i.test(volunteerId) && volunteerId !== 'PG-0000') {
    const [volunteerResult, serviceResult, applicationResult, auditResult] = await Promise.all([
      ...
      prisma.$queryRaw`DELETE FROM audit_logs WHERE operator->>'id' = ${volunteerId} OR submitter->>'id' = ${volunteerId}`,
    ]);
